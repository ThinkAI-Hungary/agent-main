  ...
  try {
      ...
      (function() {
        ...
        if (panelEl) {
          ...
          textElements.forEach(el => {
            el.style.textShadow = '0 2px 4px rgba(0,0,0,0.3), 0 0 20px rgba(0,0,0,0.1)';
    ...
    if (imageToUse) {
      await page.evaluate(`async () => {
        const img = document.getElementById('image-slot');
        if (!img) return;
...
export async function renderPolotnoJSON(json: any): Promise<string> {
  ...
  try {
    ...
    await page.evaluate(`async (polotnoData) => {
      await window.renderPolotno(polotnoData);
    }`, json);
    ...
    const layoutStats = await page.evaluate(`async () => {
      return await window.finalizeLayout();
    }`) as any;
