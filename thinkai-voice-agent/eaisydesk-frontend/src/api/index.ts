  ...
  try {
    ...
    while (attempts < 2) {
      ...
      if (model === 'auto' || model === 'flux-auto') {
        ...
        const genBgResult = await generateWithFlux2(activePrompt, 1024, 1536, { aspectRatio: '2:3', safetyTolerance: 5, guidance: 4.5, steps: 50 });
        const generatedBgUrl = genBgResult.imageUrl;
        ...
        if (productImageUrl) {
          ...
          const genRes = await generateWithFlux2(activePrompt, width ? Number(width) : 1024, height ? Number(height) : 1536, {
    ...
    for (let idx = 0; idx < campaignStrategy.items.length; idx++) {
      ...
      try {
        ...
        if (productImageUrl && imagePrompt) {
          ...
          const genCampResult = await generateWithFlux2(scenePrompt, 1024, 1536, { aspectRatio: '2:3', safetyTolerance: 1, guidance: 4.5, steps: 50 });
          finalImageUrl = genCampResult.imageUrl;
