...
export default function SocialMediaPage() {
  ...
  const platformIcon = (p: string) => p === 'instagram'
    ? <span className="sm-icon-ig">IG</span>
    : <span className="sm-icon-fb">FB</span>;

  return (
    <div className="page active">
      {/* Header */}
      <div className="mkt-page-header">
        <div className="mkt-page-header-icon mkt-page-header-icon--orange">
        ...
        <div className="flex-row gap-10">
          <button className="mkt-btn-outline mkt-btn-outline--sm" onClick={loadContent}>🔄 Frissítés</button>
      ...
      <div className="grid-2col sm-platform-row">
        <div className="mkt-platform-card instagram">
          <div className="flex-row gap-10 mb-16">
            ...
            <span className="sm-platform-name">Instagram</span>
          </div>
          <div className="flex-row sm-stats-row">
            <div><div className="sm-stat-lbl">Követők</div><div className="sm-stat-val">{analytics?.ig_followers ?? '—'}</div></div>
            <div><div className="sm-stat-lbl">Posztok</div><div className="sm-stat-val">{analytics?.ig_media_count ?? '—'}</div></div>
          </div>
        </div>
        <div className="mkt-platform-card facebook">
          <div className="flex-row gap-10 mb-16">
            <div className="sm-fb-icon">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="white"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
            </div>
            <span className="sm-platform-name">Facebook</span>
          </div>
          <div className="flex-row sm-stats-row">
            <div><div className="sm-stat-lbl">Követők</div><div className="sm-stat-val">{analytics?.fb_page_followers ?? '—'}</div></div>
            <div><div className="sm-stat-lbl">Kedvelések</div><div className="sm-stat-val">{analytics?.fb_page_likes ?? '—'}</div></div>
          </div>
        </div>
      </div>
      ...
      <div className="mkt-kpi-grid">
        <div className="mkt-kpi-card mkt-kpi-card--yellow"><div className="mkt-kpi-label">Várakozó</div><div className="mkt-kpi-value">{statusCounts.pending}</div></div>
        <div className="mkt-kpi-card mkt-kpi-card--purple"><div className="mkt-kpi-label">Jóváhagyott</div><div className="mkt-kpi-value">{statusCounts.approved}</div></div>
        <div className="mkt-kpi-card mkt-kpi-card--blue"><div className="mkt-kpi-label">Ütemezett</div><div className="mkt-kpi-value">{statusCounts.scheduled}</div></div>
        <div className="mkt-kpi-card mkt-kpi-card--green"><div className="mkt-kpi-label">Publikált</div><div className="mkt-kpi-value">{statusCounts.published}</div></div>
      </div>

      {/* Filter */}
      <div className="mkt-view-switcher mb-16">
        {STATUS_FILTERS.map(f => (
          <button key={f} className={`mkt-view-btn ${filter === f ? 'active' : ''}`} onClick={() => setFilter(f)}>{STATUS_LABELS[f]}</button>
        ))}
      </div>

      <div className="sm-post-count">
        ...
        <div className="mkt-modal-overlay" onClick={() => setShowNewModal(false)}>
          <div className="mkt-modal-card mkt-modal-card--640" onClick={e => e.stopPropagation()}>
            ...
            <div className="sm-ai-panel">
              <div className="sm-ai-panel-title">✨ AI szöveggenerálás</div>
              <div className="flex-row gap-8">
                <input value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} placeholder="Pl. Nyári fog fehérítés akció poszt..." className="sm-ai-prompt-input" />
                <button className="mkt-btn-accent sm-ai-generate-btn" onClick={handleGenerateText} disabled={aiGenerating}>
            ...
            <div className="flex-row gap-8 mb-16">
              <button className="mkt-btn-outline flex-1" onClick={handleGenerateImage} disabled={imgGenerating}>
                {imgGenerating ? '🖼 Kép generálás...' : '🖼 AI kép generálás'}
              </button>
            </div>
            {generatedImg && (
              <div className="sm-generated-img-wrap">
                <img src={generatedImg} alt="Generated" className="sm-generated-img" />
              </div>
            )}

            <div className="flex-row gap-10 justify-end mt-20">
      ...
      {showEditModal && (
        <div className="mkt-modal-overlay" onClick={() => setShowEditModal(null)}>
          <div className="mkt-modal-card mkt-modal-card--500" onClick={e => e.stopPropagation()}>
            <div className="mkt-modal-header">
