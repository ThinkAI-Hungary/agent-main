...
try {
  ...
  // ── VD1 (saját session, 'Kettes' név): szerkesztés + jóváhagyás ──
  log('VD1 modal open:', await openRow('Kettes'));
  ...
  if (await editBtn.count()) {
    ...
    log('after approve → "sikertelen" mention?', /sikertelen/i.test(bodyText));
  }
  log('close VD1 →', await closeModal());
  log('VD1 re-open:', await openRow('Kettes'));
  await page.waitForTimeout(900);
  const chip = page.locator('.ism-edited-chip').first();
  log('VD1 EDITED CHIP present:', await chip.count(), '| text:', (await chip.count()) ? (await chip.innerText()).trim() : '(none)');
  await page.screenshot({ path: '/root/.zcode/tmp/pw/33_vd1_chip.png' });
  log('close →', await closeModal());

  // ── VD2 (subject alapján): jóváhagyás szerkesztés NÉLKÜL ──
  log('VD2 modal open:', await openRow('[VERIFY] VD2'));
  if (await page.locator('.ism-card').count()) {
    await page.screenshot({ path: '/root/.zcode/tmp/pw/35_vd2_before.png' });
    await page.locator('.ism-btn-approve').first().click({ force: true });
    await page.waitForTimeout(5000);
...
} catch(e){ console.error('ERR', String(e).slice(0,600)); await page.screenshot({path:'/root/.zcode/tmp/pw/err_draft2.png'}).catch(()=>{});} finally { await browser.close(); }
