  ...
  `).join('');
    ...
    async function loadPraxisinfo() {
      try {
        ...
        if (data.price_list  !== undefined && v('tt-arlista'))     v('tt-arlista').value     = data.price_list;
    ...
    async function savePraxisinfo(sourceBtn = null, successMsg = null) {
      ...
      try {
        ...
        const price_list     = (v('tt-arlista')      && v('tt-arlista').value.trim())      || '';
        ...
        const res = await authFetch('/admin/api/praxisinfo', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ practice_name, description, address, markanev, szakterulet, kulcsszavak, megkozelites, price_list, doctors, campaigns, exceptions, modositas_eng, lemondas_24h, figyelmezteto_szoveg, pacient_id_question, new_patient_required, new_patient_auto_visit, returning_patient_required })
        });
