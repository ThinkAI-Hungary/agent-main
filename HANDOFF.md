...
- **Design finomhangolás** (commit `8c1b3c1`): az `AutomatizaciokPage.tsx` a user által adott HTML-mockup szerint lett újraístílusozva — 1120px széles konténer, breadcrumbs („Kimenő kommunikáció / Értesítések"), co-section kártyák (surface fejléc, stroke SVG ikonok tintelt négyzetben, 40×22 toggle accent-2 színnel, „Engedélyezve" felirat), nt-token chip-stílusú változók, kikapcsolt kártyánál a törzs elhalványul (opacity .55 + pointer-events none), dark mode a ThemeContext `isDark`-jából (accent-2: #186d98 → #3fd8c8). A toggle-funkció változatlan (notification-toggle endpoint). Deploy + a lazy chunk (AutomatizaciokPage-Dew5MkVo.js) tartalom szerint verifikálva; vizuális böngésző-ellenőrzés nem volt lehetséges a környezetben (nincs browser backend) — user hard refresh után látja.
