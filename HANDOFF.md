...
- **'0:00-es hívásrögzítés' incidens megoldva (`96d395c`)**: a felvételek MAGUK tökéletesek voltak (ellenőrizve: 126-128 s sztereó WAV + stream-endpoint 200). Két valós hiba: (1) lejárt JWT-nél a popup metadata-kérése csendben 401-ezett → most play-kor friss token + onError egyszeri retry; (2) az ÜGYFÉL-PROFILból nyitott popup soha nem kapott recording_url-t → most átadvva (SessionSummary típus bővítve). Élő böngészős verifikáció: lejátszó 127.82 s, lejátszik. + Prod recordings bucket konfig egységesítve (50 MB + audio mime-ek). Prod-deploy `96d395c`.
- **MARKDOWN-ELLENŐRZÉS review-agenttel (2026-09-25)**: az 5 Dentors-feladatból 3 PASS (2: Szerkesztve-chip élő teszttel + original_body megőrzés; 3: member Sürgős/Nyitott szétválasztás; 5: dashboard hármas struktúra, KPI-színek, csak-aznap-logika), 2 PARTIAL → javítva:
  - **BLOCKER (4. feladat — bubble-seek) JAVÍTVA**: a stream-végpont mostantól HTTP Range-t támogat (206 + Accept-Ranges) — 200 full-response mellett a currentTime-állítás visszapult 0-ra. Élő böngészős verifikáció: bubble-kattintás → currentTime 1.88 → 40.98, szól.
  - **MAJOR (profil-popup lejátszó) JAVÍTVA**: a useSessions flat-feed most átadja recording_url + room_name + transcript_turns (get_interactions enrich bővítve; SessionSummary/SessionInteraction típusok bővítve).
...
# 🎯 START CSOMAG — WP-E2 folytatás új sessionben (2026-09-24 éjszaka; ELSŐNEK Ezt olvasd)

**Állapot egy mondatban**: az 50 hívás ELŐKÉSZÍTÉSE megvolt, de az első ~13 próbahívás felfedte egy **kritikus worker-hibát** (a klasszifikáció spawn-olt taskként a process-kilépéskor elhalt — 12× „process did not exit in time", 0 harness-eredmény); **HOTFIX deployolva (`46f9195`**: `_run_classification` awaitelve 280 s plafonnal), a 13 hívás adatait **recovery-vel visszanyertük** (`scripts/recover_eval_calls.py`, replay sorok + ground_truth DB-ben) — az első 11 párosítható híváson az **audio-olvasat 11/11, stt 9/9 pontos, zöld precizitás 6/6, 0 rossz zöld** (nyitott: B21 pinter.szilvia.92 vs pinter.szilvia92 pontkérdés a usernek). Most: 1 próbahívással ellenőrizni az await-fixet („✅ Voice session" log + `email_verify_runs` live sor + `dry_run` email), aztán folytatni a 50 hívást.
...
- **✅ TWILIO ÉLES ÉS TESZTelve (2026-09-24 este)**: a user aktiválta a fiókot; a fiók TULAJDONA +36707177914 (magyar, SMS-képes) — beírva a Messaging Service 'easycrm' (MG82c1440cac886f770fb06b4250502cd5) pooljába, env: TWILIO_MESSAGING_SERVICE_SID. Élő teszt: SMS a +36709436426-ra → **delivered** (SM9664ade…), a status-callback (/api/public/twilio/status, aláírás-ellenőrzéssel) a sms_logs-ot delivered-re frissítette. FIX közben: az sms_logs frissítés sor-ID alapján fut (a beszúrt sor provider_sid-je NULL — provider_sid-szűrő 0 sort talált volna). Az /e/{token} oldal éles: érvénytelen tokenre barátságos hibaoldal.
- **Nyitott**: PUBLIC_CONFIRM_BASE_URL (jelenleg az admin domain esik be APP_BASE_URL-ből — hosszú távon publikus domain); P1: agent előre jelezze az SMS-t, 'all' mód, 24h emlékeztető.
...
- **EVAL_CALLER_NUMBERS=+36709436426,+36706369528** — ezekre a számokra az agent nem kap ügyfél-kontextust (`server.py _is_eval_caller`).
- **HOTFIX `46f9195`**: `_run_classification` most `await asyncio.wait_for(..., timeout=280)` — a korábbi `_spawn` miatt a folyamat kilépett, mielőtt a harness kész lett volna (gyökérok: WP-D óta spawn; LiveKit `close_timeout` 10 s).
- `email_verify_runs` tábla: minden futás (live ÉS replay) ide ír.

## 2. Teendők a következő sessionben

1. **1 próbahívás** (bármelyik eval számról, diktáljunk címet) → ellenőrzés: logban „✅ Voice session … classified" + nincs „process did not exit"; `email_verify_runs`-ban mode=live sor; `email_logs`-ban dry_run sor. HA a live sor megvan → mehet a 50 hívás.
2. 50 hívás a `tesztlista_50_hivas.csv` szerint; hívás után `idopont` + `statusz` kitöltése (a párosítás ETTŐL függ).
3. A hívások után: `scripts/replay_harness.py --csv ... --date <nap> --callers A=+36709436426,B=+36706369528` (konténerben) → baseline vs wp-e2 összesítő. A már meglévő 11+2 hívás recovery-replay sorai DB-ben vannak (ground_truth-val) — az összesítőbe beleszámítanak.
4. **USER-kérdés**: B21-nél a CSV `pinter.szilvia.92@gmail.com` (pont a 92 előtt) — a beolvasatok mind `pinter.szilvia92`-t hallottak. Mit diktáltak pontosan? Ennyi a különbség: 0 vagy 1 rossz zöld az eddigi szettben.
5. HANDOFF-ba az 50 hívás végső eredménye; élesítés (EMAIL_DRY_RUN=0) csak user-döntés után.

## 3. Nyitott tételek (user-döntést várnak)

1. **14 vs 30 nap GDPR-szöveg** összehangolása.
2. **Szélessávú HU bejövő** (Telnyx DID upstream szűksáv) — provider-döntés.
3. **Prod-deploy checklist**: `migrate_recording_columns.sql`, `migrate_interaction_count_toollog.sql`, `migrate_calendar_events_note.sql`, `migrate_client_change_log.sql`, `migrate_completion_timestamps.sql`, `migrate_email_verify_runs.sql` + prod env (EMAIL_VERIFY_MODE, EMAIL_DRY_RUN=0, SONIOX_API_KEY, EVAL_CALLER_NUMBERS NEM kell prodra).
4. Részletes stack-leírás: `WP_E_STACK_BRIEF.md`.
...
**Esemény telefonszáma = SIP-hívószám (2026-09-21, commit `584e980`)**: a book_meeting-ben a SIP-ből ismert hívószám az ELSŐDLEGES attendee_phone (volt: az LLM bemondott száma elsődleges, caller csak fallback → kamu/STT-hibás szám kerülhetett az eseményre, ld. 119: '06301234567'). Eltérő bemondott szám → custom_data.contact_phone (rokon helyett foglalás esete). A 119-es esemény kézzel javítva (+36703200236).

**Voice A–C biztonsági kör (2026-09-21/22, commit `5aba639`, stagingen ÉL, 11/11 tool-teszt zöld)**: 1) `_find_event_secure` — event_id: tulajdon-ellenőrzés (not_found, nem szivárog ki); cím: owner_email KÖTELEZŐ szűrő, a „bárki jövőbeli/legacy" fallback TÖRÖLVE a voice-ágból; 0→not_found, 1<→multiple_matches emberi jelöltekkel; `original_start_dt` másodfaktor; email-kulcs: explicit param → hívó ügyfelének emailje → need_email. 2) modify/delete naplóz (Módosított/Törölt időpont interakció + diary, csak sikeres művelet után); lemondásnál 'törölt időpont' címke + Utánkövetés (session-scope flag: session-beli újrafoglalásnál lekerül, történelmi címke sosem); új profil név alapján sosem. 3) Közös `_validate_slot` (NYITVATARTÁS — eddig csak prompt-szabály volt! — + ütközés + alternatíva), book+modify használja; modify a saját eseményét kizárja. 4) `send_followup_email` KI az ALL_TOOLS-ból; módosításnál reminder_sent reset. Tesztek: cross-client, multiple_matches, not_found, ütközés, zárva/óran-kívül, siker+napló+reset, lemondás+címke, újrafoglalás session-scope, id+tulajdon, need_email.
...
**Voice biztonság 2. kör (2026-09-22, commit `9326e34`, 5/5 teszt zöld)**: 1) név-konzisztencia a book_meeting-ben: erős-kulcs-egyezés + nulla név-token-átfedés → duplicate_suspect mindkét rekordon (konzisztens-de-hamis párok, pl. 'Lachner Ödön' rekord + 'Orosz Erika' hívó); 2) delete_meeting `confirmed` kapu: a tool először megerősítést kér az esemény-adatokkal, törlés csak confirmed=true-val; 3) tag_client az arbiteren (resolve_client_identity), létrehozás CSAK erős kulccsal (email/telefon/hívó) — puszta névvel soha. A slot-UI (ellátónkénti rendelési idő, per-doctor ütközés) TUDATOSAN későbbre halasztva (user-restancia).

**Szolgáltatás-szintű foglalási szabályok (2026-09-22, commit `d339c9d`)**: a services.note mező UI-szerkeszthető lett („Megjegyzés (foglalási szabály)" oszlop); a higiénia-mentesség az exceptions-ből a Fogkő-eltávolítás note-jába költözött (a Kivételek lista ismét tiszta eszkalációs lista — altatás stb.); a kódba égetett higiénia-KIVÉTEL sor kivezetve, generikus „note > általános szabály" pointer helyette; render-címke [Foglalási szabály: …]. Elv: provider-specifikus szabály = provider-adat, nem kód és nem az eszkalációs kivétel-lista.

**'Befoglalt időpont' az interakcióhöz tartozó esemény (2026-09-22, commit `3ac19f1`)**: a popup matchedEvent-választása a legkésőbbi (start_dt DESC) eseményt mutatta — több jövőbeli foglalásnál a korábbi ügy dátuma jelent meg (132 vs 119). Most az esemény created_at-je a legközelebbi az interakció időpontjához.

**Kézi naptár create/delete 500 fix (2026-09-22, commit `bfff340`)**: a kézi naptárfelvétel és -törlés SYNC endpointjaiban `asyncio.create_task` futott (email-ütemezés) → „no running event loop" 500 a sikeres DB-művelet UTÁN. Tünet: „Hiba törléskor" toast + a lista nem frissült, pedig a törlés megtörtént. Mindkét endpoint `async def` lett. E2E: create 200 + delete 200. Tanulság: sync FastAPI-endpointban nincs event loop — `asyncio.create_task` helyett async def (vagy thread/queue).

**Kézi naptár-törlés 'törölt időpont' rituáléval (2026-09-22, commit `9dddd71` + `b1ec547`)**: a kézi törlés a kapcsolódó ügyfélhez (email→telefon→név azonosítás) a voice/cancel-link flow-val azonos rituálét alkalmazza: 'törölt időpont' címke (sales-tag → első oszlop) + Utánkövetés státusz + interakció (eredmény: Törölt időpont) + ügyfél-napló. Hibás felvitelnél a címke manuálisan eltávolítható; azonosíthatatlan ügyfélnél nincs címkézés. Az FK-fix: a napló-sessiont create_session-nel kell létrehozni (23503-as FK-hiba volt). E2E: tag + utánkövetés + interakció mind megjelent.

**Alapelvek-csere + kódoldali CSENDES azonosítás (2026-09-22, commit `2f83b80`)**: 1) A find_client kivéve a modell kezéből a hívás elején (a GDPR-hozzájárulás előtt hívta — a 0. szabály konfliktusa okozta a minőségromlást); a szerver oldja fel resolve_client_identity-vel és CSENDES profil-hintet injektál („NE szólítsd név szerint, NE olvasd be — csak szükség esetén: foglalási email-megerősítés, modify/delete tulajdon-check; érzékeny adat csak második azonosító után"). 2) ALAPELVEK (user 6 pontja + report_alert névvel) a DB-template tetejére + seed. 3) patient_rules lecsupaszítva: 0/5/6/8 törölve; 1–4 (provider-adat), 7. (PSTN), 9. (modify/delete) maradt. Render-ellenőrzés zöld minden pontra. A „túl sok egyenrangú SZIGORÚ szabály" dilúció megszüntetve — az alapelvek most DB-adat, a prompt hierarchiája tiszta.

**Telefonszám-normalizálás az arbiter egészére (2026-09-22, nyitott tétel #7)**: a kanonikus `normalize_phone_digits` (utolsó 9 számjegy, 06→36, szeparátor-mentes) a tools.py-ból a database.py-ba költözött — a tools `_normalize_phone_digits` most vékony wrapper rá. 1) `db.find_clients_by_phone` (lista) + `_find_client_by_phone` (single): kétlépcsős egyezés — gyors ilike a 9-jegyű szekvenciára, ha nincs találat, full-scan fallback a telefonnal rendelkező ügyfelekre (a szeparátoros tárolás — '06 30 555 4433' — NEM tartalmazza a '305554433' substringet, az ilike önmagában elszalasztaná); a normalizált egyenlőség a substring-hamis-találatokat kiszűri. 2) `find_client_by_contact` átírva: a korábbi pontos `phone.eq` (és az email+phone OR/id-desc, ami nem-determinisztikus volt) helyett determinisztikus telefon > email sorrend, normalizált telefon-egyezéssel — így az arbiter (resolve_client_identity), a csendes azonosítás (server.py), a book_meeting caller-feloldás, az upsert_client és a web_server:3417 mind formátum-függetlenek lettek. 3) find_client tool (tools.py) a `db.find_clients_by_phone`-t hívja (a saját ilike-ja kikerült). 4) Ügyfél-törlés kaszkád: a session-participant ilike most a normalizált 9 jeggyel fut. Tanulság: substring-egyezés telefonra csak normalizált post-filterrel biztonságos, és a szeparátoros tárolást az ilike NEM fedi — kétlépcsős megoldás kell. Tesztek: `tests/test_client_identity.py` (11 új — normalizálás, formátum-függetlenség, telefon>email prioritás, substring-hamis-találat, arbiter út), 96/96 zöld.

**Szolgáltatás-tábla: 'Leírás' oszlop kivezetve (2026-09-22, user-kérés — „redundáns")**: a Foglalható szolgáltatások, kollégák szekció 4 oszlopos lett: Szolgáltatás neve | Időtartam (perc) | Kolléga | Megjegyzés (a fejléc a „Megjegyzés (foglalási szabály)"-ról rövidült, a cél-layout a user csatolmánya szerint). A services.description DB-oszlop és a meglévő adatok ÉRINTETLENEK (a mentés-payload továbbra küldi őket) — de a description a voice prompt renderből is kikerült (prompt_utils._format_services), különben UI-n láthatatlan szöveg befolyásolná tovább az agentet. A duration/assigned/note rendszer-szerepe (resolve_service_duration, resolve_assigned_staff, [Foglalási szabály] render) változatlan. Verifikáció: tsc 0 hiba, pytest 96/96, stagingre deployolva + Playwright-screenshot: a fejlécek pontosan a 4 oszlop, adatok érintetlenek, konténer healthy, 0 ERROR.

**Kézi naptár-CRUD rendberakás (2026-09-22, user-jóváhagyott terv alapján, 7 tétel)**: 1) **Létrehozás-500 gyökér**: az admin_create_event auto-ügyfél blokkja `_auth`-ot hívott `_user` helyett (NameError) → az esemény létrejött, a toast mégis hibát mutatott, ÉS az ügyfél-profil sosem jött létre (innen a „Ügyfél nem található" toast a listából). 2) **Ügyfél-létrehozás az arbiterrel**: a naiv get_clients-szkennelés helyett resolve_client_identity (normalizált telefon is); létrehozás CSAK erős kulccsal (email/telefon, a voice tag_client szabálya); konfliktnál mark_duplicate_suspect mindkét rekordon (merge-banner); a blokk try/except-ben — az esemény 500-ba sosem fordulhat. 3) **Szolgáltatás-dropdown + „Egyéni…"** a kézi popupban: a services-listából, üresen nem hagyható (kötelező validáció), választáskor az időtartam autofillel a tábla percével (felülírható); „Egyéni…" free-text inputot nyit (a voice/email flow-t nem érinti — az a backendet hívja). Szolgáltatás-listás címnél a backend `_service_normalized_title` az egységes '<szolgáltatás> - <név>' formára hoz + a TÁBLA időtartama nyer; egyedi cím érintetlen. Szerkesztéskor a '<szolgáltatás> - <név>' címből a név-suffix levágva, a szolgáltatás előre kiválasztva. 4) **Megjegyzés (note) mező** a popupban (calendar_events.note — staging migráció Management API-val lefutott; prod: `migrate_calendar_events_note.sql` KÖTELEZŐ). 5) **Törlés**: a másodlagos „értesítés?" modal ELMARAD (user-döntés) — a lemondási email mindig kimegy; a 'törölt időpont' rituálé változatlanul fut (E2E: címke + utankovetes státusz + interakció + napló mind zöld). 6) **Módosítás-email**: old_datetime a DB-beli RÉGI start_dt (volt: a form új értéke ment ki régi gyanánt); kézi módosításnál KIZÁRÓLAG a módosítás-visszaigazoló megy (E2E: 1 update = pontosan 1 email — a user által jelzett dupla küldés a jelenlegi kódból nem reprodukálható). 7) **Értesítő-emailek auditja**: confirmation/modification/cancellation küldések mostantól az email_logs táblába is kerülnek (volt: csak log_outbound_message — visszaellenőrizhetetlen volt). 8) **Plusz talált bugok**: kézi felvétel tz-shift (naive form-idő UTC-ként tárolódott → +2h a gridben; most Europe/Budapest-ként parse-olódik) + attendee_phone a kézi eseményre nem mentődött (a törlés-rituálé azonosítása miatt fontos) + update None-értékek nem nullázzák a mezőket (volt: assigned_to nélküli szerkesztés letörölte az ellátót). E2E mind zöld (create 200 + ügyfél 291 + normalizált cím + tábla-időtartam + note; update 1 email; delete rituálé + lemondás-email; egyéni cím verbatim 20 perc). Tesztadatok törölve. tsc 0, pytest 96/96, konténer healthy.

**Név-eltérés jelzése kézi naptárfelvételnél (2026-09-22, user-teszt: 'Oros Erika' elgépelés)**: a kézi create/update válasz most `name_hint`-et ad, ha az arbiter ERŐS KULCCsal (email/telefon) talál meglévő ügyfelet, de a beírt név ékezet-hajtva (_accent_fold, NFKD) sem egyezik a tárolt névvel (részleges begépelés — 'Orosz' ⊂ 'Orosz Erika' — megengedett, nem jelez). Frontend: info-toast („a beírt név eltér a tárolttól — a foglalás a meglévő profilhoz kapcsolódik"). Szándékosan NEM hoz létre új profilt és NEM javítja át a nevet (a rokon-eset legitim, az email=erős kulcs dönt). A név-konzisztencia duplicate_suspect (9326e34) továbbra is csak nulla token-átfedésnél jelez — ez a hint annál lazább, UI-visszajelzés, nem merge-jelölés. E2E: elgépelt név → hint; pontos név → nincs hint; új ügyfél → nincs hint. User-döntés: a profil „Időpontok" szekció (csak legközelebbi jövőbeli) NEM bővül — fogászaton nem életszerű a több párhuzamos jövőbeli időpont.

**Értesítési központ rendberakás (2026-09-22, user-jelzés)**: 1) A pulzáló glow kivezetve (`.notif-bell-pulse` JSX + CSS törölve) — marad a piros számbadge. 2) **A „megállás nélkül jelez" gyökere**: a csengő a NYERS `/admin/api/interactions`-ből soronként számolt (egy hívás = átirat + tool-log sorok külön olvasatlan elemek), míg a lista a grouped végpont reprezentatív id-jét jelölte olvasottnak → a listás megnyitás a csengőt nem csökkentette (és a tool-sorok is beleszámítottak a badge-be — innen a 47). A csengő most a **grouped végpontot** pollozza: 1 thread = 1 értesítés, az olvasottság a reprezentatív interakció id-jén — ahol kinyitod (lista VAGY csengő), mindkét helyen olvasott. Mellékhatás: threadbe érkező új üzenet a reprezentatív id megváltozásával újra olvasatlanná teszi a threadet (kívánatos). E2E: badge 47 → 26 (thread-szint), pulse 0 elem, listás megnyitás → badge 26 → 25 a következő pollban.

**Változási napló az ügyfélprofilon (2026-09-22, user-spec + látványterv alapján)**: új `client_change_log` tábla (tenant, client_id, actor, action, label, new_value, related_ref, created_at; RLS a meglévő minta szerint — migráció stagingen lefutott, prod: `migrate_client_change_log.sql` KÖTELEZŐ). A v1 SZÁNDÉKOSAN nem tárol régi értéket (user-döntés), és az AI/upsert mező-írások NEM naplózódnak (csak szándékos emberi módosítások + az időpont-életciklus „eaisyDesk" aktorral). Instrumentált pontok: profil-PUT diff (címke +/- és mező-módosítás, HU mezőcímkékkel, belső kulcsok kizárva), kanban státusz-mozgatás (oszlop-név feloldással), kézi teendő +/- , időpont létrehozva/módosítva/törölve (kézi úton az aktuális user, voice/email/messenger úton „eaisyDesk"). API: GET /admin/api/clients/{id}/changes (member is olvassa — user-döntés). Frontend: history (rotate-ccw) ikon a profil-fejléc kebabja BALRA, középre igazított modal a látvány szerint; kereső 300 ms debounce-szal, kis/nagybetű- és ékezet-érzéketlen (NFD-fold), keres: eseménynév / módosító / új érték / kapcsolódó hivatkozás. E2E: mind a 10 akció-típus sor megjelent helyes aktorral; keresés („telefonszám" → 1 sor) működik; vizuális screenshot a látvánnyal egyező. Aktor-feloldás: full_name → username → „eaisyDesk" (_actor_name helper).

**Változási napló — gomb-stílus a látványterv szerint (2026-09-22)**: a profil-fejléc gombpárja (history + kebab) közös `cd-hero-actions` flex-wrapperbe került (10px gap — a history a kebabtól KÖZVETLENÜL balra, korábban a pill mellett szökött); mindkét gomb azonos fehér/téma-tile (40×40, 12px sarok, vékony border+árnyék). A kebab stílusa KÉT css-ben is létezett (clients.css ÉS clientprofile.css — utóbbi nyer a betöltési sorrendben): a tile-stílust mindkettőbe beírtam, a kettős definíció a jövőben forrása lehet eltérő méretnek.

**288-as ügyfél 4 problémája + aliasz-osztály gyökérfix (2026-09-23, user-teszt)**: 1) **Fantom „telefonszám módosítva" a változási naplóban**: az email-pipeline `phone`, a profil-form `telefonszam` kulcsra ír → a changelog-diff kulcsszinten összevetett. Most `_CHANGELOG_ALIAS_GROUPS` (telefonszám: telefonszam/phone/telefon, név: name/nev) — érték-szintű összevetés, jelzés csak valós változásra. 2) **Telefonszám-normalizálás (user-szabály: egyféle forma mindenhol)**: `db.normalize_phone_hu` — kanonikus '+36 XX XXX XXXX' (mobil 9 jegy) / '+36 1 XXX XXXX' (vezetékes); 06/0036/prefix-nélküli bemenet kezelve, külföldi/anomál számok érintetlenek. Bekötve írásoldalon: upsert_client, add_client, edit_client_details (minden rendszer- és kézi út). Backfill: 5 meglévő ügyfél normalizálva (233/149/176/288/274). 3) **Plusz talált adatvesztés-bug**: az edit_client_details a phone OSZLÓPOT a puszta custom_data.get("phone")-ból írta → a 'telefonszam'-aliasszal mentő profil-form KINULLÁZTA az oszlopot (és az arbiter pont ezen keres). Most az oszlop az aliászok első nemüres értékéből töltődik. 4) **Email topic csonkolás**: volt `Email AI válasz - {subject}: {body[:200]}` (a prefix is a keretbe esett → a név lemaradt); most `Bejövő e-mail — {subject}: {body[:80]}…` — a teljes törzs változatlanul a diary_fragmentben. A frontend topic-fallback MINDKÉT formátumot parse-olja. 5) **Rokon-foglalás (288)**: a kliens neve eddig a tartalomból kinyert (beteg) név volt — most a From-fejléc megjelenített neve (a tényleges küldő) az elsődleges, kanban.name csak fallback. NEM volt szándékos kontextusfigyelés: az anchor a feladó-email, a név forrása viszont a tartalom volt. E2E: létrehozás normalizálva, név-only PUT → csak név-sor a naplóban, phone-oszlop megmarad. tsc 0, pytest 96/96. (Az 1)/4)-es pontok a következő ÉLŐ email-teszttel igazolandók viselkedésben is.)

**Időpont-modal redesign open-design spec alapján (2026-09-23, user designkód)**: a naptár Új időpont/Szerkesztés modal a csatolt open-design mintára építve: 560px, felülre igazított overlay (rgba(0,0,0,.42), 64px padding, blur NÉLKÜL — az app többi modaljához nem nyúltunk, dedikált appt-overlay osztály), 2-oszlopos appt-grid (Ügyfél neve|Email / Telefon|Munkatárs / Szolgáltatás full / Egyéni full (feltételes) / Dátum|Időpont / Időtartam / Megjegyzés full), 36px inputok #186D98 fókusz-gyűrűvel, NEM-NATÍV dropdownok (új shared `Cdd` komponens — trigger+chevron, abszolút menü, pipa az aktívon), lábléc: Törlés (danger) balra, Mégse + Mentés/Létrehozás (primary #186D98). Minden handler/state érintetlen (serviceSel, autofill 45 perc, name_hint toast, validáció); téma-változókon fut (dark mode kompatibilis). A mockup fiktív adatai (staféta-nevek, APPT tömb) NEM kerültek be. Vizuális verifikáció: új/edit modal + nyitott dropdown screenshotok a mockuppal egyezők; Konzultáció → 45 perc autofill; Egyéni mező megjelenik; szerkesztésnél prefillelt mezők + törlés-gomb.

**Módosítás-email csak ütemezés-változásra + Budapest-prefill (2026-09-23, user-teszt: note-szerkesztésre is visszaigazoló ment)**: 1) **A gyökér**: a kézi update-ág MINDEN mentésnél feltétel nélkül küldte a módosítás-visszaigazolót. Most ütemezés-releváns diffhez kötve (start/end/duration/cím/ellátó — instant-összevetéssel); note-only vagy no-op mentésre NEM megy email. Edge: az üres assigned_to „nem nyúlt hozzá"-nek számít (a régi értékkel helyettesül az összevetésben — különben minden munkatárs-nélküli mentés „ellátó-változás" lenne). 2) **Súlyosabb rejtett bug**: az openEventEdit a prefillt NYERS UTC-szövegből szeletelte (a 10:00-s bp-i esemény 08:00-ként jelent meg) → note-only mentés csendben 2 órával eltolta volna az eseményt. Most Intl/ Europe/Budapest konverzió (éjfél-átlépés is helyes). 3) **Változási napló finomítás**: note-only módosítás „Megjegyzés módosítva" címmel és a note-szöveggel naplózódik (nem „Időpont módosítva" dátummal). E2E: note-only → 0 email + „Megjegyzés módosítva" sor; időpont-módosítás → +1 email + „Időpont módosítva" sor; prefill a valós 137-es eseményen 10:00. tsc 0, pytest 96/96, konténer healthy.

**Naptár-szerkesztés: telefon-persist + profil-kitöltés + feltételes normalizálás (2026-09-23, user-teszt — Balogh Márta)**: 1) **attendee_phone az update-ben nem mentődött** (se átadás, se allowed-lista) — most mentődik, ráadásul normalizálva (create + update is). 2) **Profil-telefon kitöltés a naptárról**: `_fill_client_phone_if_empty` — ha a kapcsolódó ügyfél telefonja üres, a panelen beírt szám rákerül a profilra (normalizálva) + changelog profile_edit; meglévő számot SOSEM ír felül. Bekötve create + update ágba. 3) **Fantom módosítás-email gyökére**: a szerkesztés vakrepülésben futtatta a cím/időtartam-normalizálást — a legacy 'Konzultáció' cím → 'Konzultáció - Név' + 30→45 perc változás triggerelt emailt. Most a normalizálás CSAK tényleges szolgáltatás-/címváltásnál fut (accent-fold összevetés a név-suffix nélküli base-címmel); telefon-/note-szerkesztés az ütemezéshez nem nyúl → nincs email. E2E: phone-only → 0 email, esemény+profil normalizált telefonnal, changelog sor; időpont-váltás → +1 email. tsc 0, pytest 96/96.

---

## ✅ 2026-09-24 — Balázs-scope Wave 1 (Dentors kommentek #1, #2, #3+#5) — commit `c2e277d`, stagingen ÉL

A `balazs_backlog_0922.md` mellett a Dentors-kommentek táblából (Státusz=Balázs) 3 munkacsomag subagent-driven fejlesztéssel, a jóváhagyott terv + Tesztelési Kapu (DoD) szerint. **Pre-flight**: `git pull` tiszta (HEAD `9576eb4`), horgony-reverifikáció. ⚠️ **MCP-csapda MEGERŐSÍTVE**: a Supabase MCP default projektje a PROD (`dsiluafthysysnstszbd`) — minden staging-DDL/SELECT a Management API-n futott (`POST /v1/projects/qhhnqqsthdrwacsxommt/database/query`, sbp token). A staging-identitást a `client_change_log` megléte igazolta (prodon még nincs).

**WP-A — nem-páciensek szűrése (JEV via OpenRouter)**: új `jev_classifier.py` — `classify_sender()` az OpenRouter **Decisions** végponton (`/api/alpha/decisions`, modell `typesafe/jev-1.13`, EGY choice-kérdés: paciens|munkatars|szolgaltato|marketing, per-label valószínűségek); 429/5xx/timeout backoff (2 retry), **fail-open** (hiba/confidence < `SENDER_FILTER_THRESHOLD` (0.8) → páciensként megy tovább). Gate a `process_single_email` spam-kapuja mögött (`asyncio.to_thread` — az eseményloop nem blokkol): találatnál session `filtered_<email>` + interakció `tool_name='sender_filter'`, `funnel_stage='non_patient'`, `approval_status='spam'` — ügyfél NEM készül, AI-válasz NEM megy. `get_interactions` + `get_grouped_interactions` RPC kiszűri a `non_patient`/`spam` funnel-sorokat (a korábban átszivárgó spam-sorok is eltűntek a naplóból). Új env: `OPENROUTER_API_KEY` (csak .env!), `OPENROUTER_JEV_MODEL`, `SENDER_FILTER_THRESHOLD`. Migráció: `migrate_interaction_count_toollog.sql` új verziója **stagingen lefutott; prod-deploynál KÖTELEZŐ**. ÉLŐ E2E (konténerből, rivergate tenant): marketing conf=1.0 → **szűrve**; páciens conf=1.0 → teljes feldolgozás (pending draft — a stale-offer őr visszatartotta az autonóm küldést); munkatárs (`balazs@thinkai.hu`) conf=**0.78** → a 0.8-as küszöb alatt **átengedve** (tervezett konzervatív viselkedés — kalibrációs jegyzet: élő levelek alapján a küszöb emelhető/csökkenthető). Vizuális: a naplóban a „Szűrt feladó" keresésre „Nincs találat".

**WP-B — szerkesztett vs. generált válasz**: új pure helper `email_processor._stamp_draft_meta(draft, final_text, sent_by, edited_by, channel_texts)` — `original_body` (első hívás nyeri, idempotens), `edited` (sticky, whitespace-diff nem számít), `edited_at` (Budapest), `edited_by`, `sent_by` ('human' approve | 'ai' autonóm — email + meta ág is), multi-channel támogatás. Approve-végpont (`_stamp_approved_draft`) minden csatornára (email/wa/messenger/ig/telefon) stampel; `edited_by` = JWT username. **email_logs stale-pending javítás**: `db.mark_email_log_sent(session_id, to_email)` — a draft-időben írt pending sor approve utáni sikeres küldésnél sent-re fordul (try/except, sosem dönti el a jóváhagyást). Frontend: a popup „Elküldött válasz" buborékja mellett **„Szerkesztve · <név>" chip** (`.ism-edited-chip`, világos/sötét téma). ÉLŐ E2E: a 900-as draft szerkesztve jóváhagyva → `edited=true, edited_by=admin, sent_by=human`, `original_body` érintetlen (az AI eredeti szövege), `email_logs` → `sent`, és a WP-C `closed_at`-je is beíródott. Vizuális: chip a popupban („Szerkesztve · admin") ✓.

**WP-C — irányítópult, 1. megoldás (Dentors #3 + #5)**: migráció `migrate_completion_timestamps.sql` — `interactions.closed_at` + `tasks.completed_at` (**stagingen lefutott; prod-deploynál KÖTELEZŐ**). Close-endpoint ÉS `update_approval_status` (approve→Lezárt) írja a `closed_at`-et (oszlop + classification JSON); `update_task_complete` írja a `completed_at`-et (visszavételnél NULL) — a changelog-hook érintetlen. Frontend: `MemberDashboardPage` hármas struktúra — **Sürgős teendők / Nyitott teendők / Ma elvégzett** (Lezárt `closed_at` ma + elvégzett kézi teendők; üres állapot: „Ma még nincs elvégzett teendő"); lejárt (nyitott, created<ma-éjfél) = a Nyitottban **„Lejárt" badge**; legfrissebb felül; KPI: Sürgős (piros) / **Nyitott sárga ikonnal** / **Ma elvégzett zöld pipával** / Mai időpontok változatlan. `useSessions` mező-tükör: `closed_at` (28. tétel tanulsága). Ismert korlát: a `closed_at` NÉLKÜLI régi lezárt sorok sosem jelennek meg a Ma elvégzettben. ÉLŐ E2E: `member_test` (member role!) zárás → `closed_at` + `closed_by=member_test` ✓; Vizuális (light+dark screenshotok): 4 KPI, szekciók, Lejárt badge-ek, Ma elvégzett sorok ✓. Új `tests/audit-dashboard.spec.ts` (mockolt sorok, „másnap üres" eset is).

**Smoke-fix (előzetes hiba, nem a mi regressziónk)**: a `/admin/automatizaciok` smoke a `.page-title` anchorért elbukott — a stash-teszt igazolta, hogy a `9576eb4` HEAD-en is elbukik. Az AutomatizaciokPage h1-e inline-stílusos, osztály nélkül → `className="page-title"` felvéve (minden tulajdonságot az inline felülír — vizuális változás NINCS).

**Tesztelési Kapu eredmények**: pytest **96→123 zöld** (20 sender-filter + 7 draft-meta); `tsc --noEmit` 0; vite build OK; Playwright **35 passed / 0 failed** (5 visual skipped — SHOTS-mód); staging deploy → konténer **healthy, 0 ERROR**, 4 kulcsfájl md5-egyezik a repóval; élő E2E-k fent; vizuális screenshotok: `dash-light/dark-full`, `dash-ma-elvegzett`, `naplo-list`, `naplo-szurt-kereses`, `popup-szerkesztve-chip`.

**Talált, NEM javított (user-döntés kell)**: **149/178 duplikátum** — mindkettőn `balazs@thinkai.hu` (149 „Lederer Balázs" voice-forrású, 178 „Kis Lajos" júniusi email-forrású); az email-pipeline `find_client_by_contact`-ja az újabb (178) rekordot frissíti. A merge-felületen összevonandó (mint 265/271). Kisebb jegyzet: a popup a `sent_at`-et UTC-ben mutatja (előzetes megjelenítési út).

**Takarítás**: minden Wave-1 tesztadat törölve stagingen (900–903 interakciók, 4 session, 297-es tesztügyfél, email_logs sorok; 178 naplójának teszt-blokkja visszavágva) — a 149/178 pár érintetlen a merge-döntéshez.

**Nyitva**: **WP-D (hangrögzítés, Dentors #4)** — kód flag mögött (`RECORDINGS_ENABLED=0`); EXTERN ELŐFELTÉTEL: LiveKit Cloud egress engedélyezése + Supabase Storage S3-kredencialek (.env: `RECORDINGS_S3_*`) + privát `recordings` bucket. Prod-deploy checklist bővült: `migrate_completion_timestamps.sql`, `migrate_interaction_count_toollog.sql` (új), + a kolléga `calendar_events.note` / `client_change_log`. Kalibráció: sender-filter küszöb élő levelek alapján (jegyzet: 0.78-os munkatárs eset átesett a 0.8 alatt).

**✅ WP-D elő-ellenőrzés élesben (2026-09-24, user kérésre)**: az egress a LiveKit Cloud Build planen **működik, külön engedélyezés NEM kell** — bizonyíték: `list_egress` válaszol, és valódi `RoomCompositeEgressRequest` (audio-only) **elindult** (`EG_3ZaPcKcoSNgV`, 15 mp-es próba, dummy S3-creds). ⚠️ Két technikai tanulság: (1) a telepített `livekit-api 1.1.0`-ban a dispatch mező `room=` (NEM room_name), és a Cloud a `file=EncodedFileOutput(...)` oneofot várja; **destináció NÉLKÜL (puszta filepath) a Cloud ELUTASÍTJA az egress-t** ("missing or invalid field: output") → S3-hely KÖTELEZŐ, a Supabase Storage S3-végpontja a cél. (2) a composite egress belövése lassú (15 mp alatt nem ért ACTIVE-be — valódi hívásnál nem gond). **Megtörtént**: privát `recordings` bucket létrehozva Storage REST API-val (service key, dashboard NÉLKÜL: 50 MB limit, audio MIME-ek). **Egyetlen hiányzó tétel**: Supabase Dashboard → Storage → S3 access keys kulcspár (account-szintű, API/MCP-ből NEM hozható létre — kutatva) — vagy bármilyen S3-kompatibilis tároló kredencsek. Érkezéskor: `.env` (RECORDINGS_S3_ENDPOINT/KEY/SECRET/BUCKET) + `RECORDINGS_ENABLED=1`, utána a Wave-2 implementáció. Probe-tesztadatok törölve (2 session + 2 interakció).

## ✅ 2026-09-24 — WP-D LEZÁRVA: hívásrögzítés + lejátszó (commit `2a38bca`+`3982a8c`+`d2e1707`, stagingen ÉL)

**Fontos architektúra-döntés (élő probe-okkal igazolva)**: a LiveKit Cloud **RoomCompositeEgress ezen a projekten NEM indul el** — 3 kísérlet (üres szoba / agent-szoba / 3 perc türelem) mindegyike EGRESS_STARTING-ban ragadt vagy ABORTÁLT ("Start signal not received"); TrackComposite-hez a probe-szobákban nem volt publikált track. Ezért a rögzítés **agent-oldali capture-vel** készült: a worker maga csapolja mindkét hangirányt — NEM igényel egress-t ÉS az S3-kulcsokat sem (a .env-ben maradtak `RECORDINGS_S3_*`-ként, tartalékul). A "Build planen az egress megy" várakozás a composite-re nem teljesült; ha később a LiveKit dashboardon rendeződik, az egress-út külön újraválasztható.

**Megvalósítás**: új `call_recorder.py` — `CallRecorder` (hívó: `track_subscribed` → `rtc.AudioStream`; agent: RoomIO `LocalAudioTrack` poll 120 s-ig — a track az első beszédnél publikálódik; 20 ms-grid writer, silence-fill, drop-oldest; **WAV 16 kHz/16-bit sztereó: hívó BAL, agent JOBB**); pure helperok tesztelhetők livekit nélkül. Upload a privát `recordings` bucketbe (`<tenant>/<dátum>/<session>.wav`, `db.supabase.storage` — avatars-minta). DB: `sessions.recording_url` + `interactions.transcript_turns` JSONB (stagingen lefutott, `migrate_recording_columns.sql`; **prod KÖTELEZŐ**); `log_interaction` transcript_turns paraméter fallback-droppal; grouped enrichment visszaadja a `recording_url`-t. Végpont: `GET /admin/api/sessions/{id}/recording` (verify_jwt → 1 órás signed URL). Frontend: popup **„Hívásrögzítés" lejátszó-sor** (play/pause + idő, lazy signed-URL fetch), és ha van `transcript_turns`, **bubble-szintű seek-ikonok** (Felhasználó/AI buborékon → odaugrik a hang). Retention: a reminder-worker napi futása törli a `RECORDINGS_RETENTION_DAYS` (30) napnál régieket + nullázza az URL-t. Flag: `RECORDINGS_ENABLED=0` esetén minden inaktív.

**Élő E2E (deployolt workerrel, probe-hívás)**: 105 mp-es szoba → WAV feltöltve (6,68 MB, 0 dobott minta) → `recording_url` beírva → signed URL HTTP 200 (audio/wav) → popup lejátszó vizuálisan ✓ (screenshot: `popup-recording-player.png`). Közben 2 hiba javítva: `track_publications` lista/dict kettős kezelés (SDK-verziófüggő) + a bucket `allowed_mime_types`-ába az `audio/wav` felvétele (415-ös feltöltési hiba). Probe-agent-csatorna probe-körülmények közt csendes volt (a track késve publikál — poll 30→120 s-ra bővítve).
...
## ✅ 2026-09-24 — WP-E LEZÁRVA: post-call email/név verifikációs harness (commit `95c90e3`, stagingen ÉL, `EMAIL_VERIFY_MODE=1`)

User-döntések: verifikáció = **név + email** (telefon → SIP caller-ID ground truth, kihagyva); **mindig autokorrekció** a legjobb kandidátusra (audit-lábbal); nem-zöld esetén **verify-then-send dupla opt-in**; második STT = **ElevenLabs Scribe v2** batch (csak).

**Megvalósítás**: `email_verify_harness.py` (~750 sor): magyar spoken→written normalizáló (kukac→@, pont→., dupla X, szám-szavak, ékezet-variánsok), email+név kandidátus-generátor (accent-fold, domain-whitelist Levenshtein ≤2: gmail/freemail/citromail/indamail/outlook/hotmail/yahoo), JEV arbitráció (OpenRouter decisions, ≥0,99 → autonóm), MX/A-record validáció, Scribe v2 kliens (multichannel — a sztereó WAV-on hívó/agent KÜLÖN átirat, word-szintű logprob, keyterms). Folyam: `book_meeting` verify-módban STASH-eli a foglalást (visszaigazoló NEM megy ki azonnal) → hívás vége → `_run_classification` végén harness-spawn (`asyncio.to_thread`) → felvétel letöltés → Scribe → normalizálás mindkét átiraton → kandidátusok → arbitráció → **mindig alkalmazza a legjobbat** (client email + events attendee_email + audit `custom_data.email_verification/name_verification = {previous, value, confidence, source, status, ts}` + interakció-note korrekciónál) → GREEN (szintaxis ∧ (egyezik ∨ conf≥0,99) ∧ mx≠False) → visszaigazoló megy; NON-GREEN → **dupla opt-in levél** (`GET /api/public/verify-email?token=` — kattintásra megy a visszaigazolás + interakció-napló). Minden út fail-open (harness-hiba → régi azonnali visszaigazolás). Régi viselkedés: `EMAIL_VERIFY_MODE` unset/0 → bájtonkénti azonos.

**Élő bizonyítékok**: kulcs-probe Scribe → 200; valódi felvétel multichannel átirata (hívó=ch0, 230/230 szó logprob); TTS golden szett (Gemini TTS — az ElevenLabs kulcson NINCS TTS scope, 401) top-1 6/10, a 4 tévesztés akusztikai szintű (pontosan az eltérés-osztály, amit a live-Gemini + JEV kezel); JEV kalibráció (domain-vita 0,6, név-vita 0,26 → helyesen non-green). **Valódi hívásos E2E** (Lederer Balázs, 92 mp): mindkét csatorna rögzült (hívó peak 32762/17 hangos mp, agent 28834/48), `transcript_turns`=8 offsetekkel, popup lejátszó + 8 bubble-seek ikon, kattintás → betöltés+seek+lejátszás OK. Verify-endpoint E2E: token → „Köszönjük! Az e-mail cím megerősítve." + interakció-napló + visszatartott visszaigazolás kimegy (email_logs: clicked → sent).

**Közben javított hibák** (élő verifikáció alatt): `track_publications` lista/dict; bucket `audio/wav` MIME; agent-track poll 30→120 s; bubble-seek holtpont (preload="none" + React src-race → imperatív src+load+play); `onLoadedMetadata` pooled-event null currentTarget → popup-krash; naplós popup telefonnál diary-parse üres → bubble-ök a `transcript_turns`-ból; lejátszás same-origin streamre (`?token=` JWT, a külső signed URL-t a médiaelem nem nyitotta meg). Lemez: build előtt `df` + `docker builder prune` (97%→78%).

**NYITVA**: (1) valódi **email-diktálásos** teszthívás a teljes non-green lánchoz (verify-then-send kattintással); (2) **tartalom-inkonzisztencia**: az agent GDPR-szövegben 14 napot mond, a retention 30 (`RECORDINGS_RETENTION_DAYS`) — összehangolandó; (3) **Soniox mint opcionális 3. motor**: élő probe a valódi felvételen működik (WS `stt-rt.soniox.com`, `stt-rt-v4`, token-szintű confidence — a hívót „Szabolcs vagyok"-nak írta, miközben a nyilvántartás „Balázs" — motorok közti név-variancia, a harness célpontja); hozzáadása konfig-mel bővíthető. Prod-deploy: `migrate_recording_columns.sql` + WP-E (séma-változás nincs!) + `EMAIL_VERIFY_MODE` bekapcsolása csak a staging-tapasztalat után.

**🔊 Audio-lánc audit (2026-09-24)**: a rögzített hívások rossz minőségének gyökere feltérképezve → [HANGFELVETEL_AUDIT.md](HANGFELVETEL_AUDIT.md). Fő megállapítás: a hívó lánc **G.711 szűksáv** (FFT-mérés: 3400 Hz felett 0,01% energia), az agent szélessávú; javítandó: Telnyx FQDN connection-en G.722 engedélyezése (nincs eltárolt Telnyx API kulcs!), rögzítő idővonal-hibák (tail-append + wall-clock grid), Krisp-stacking. Részletek a fájlban. Utána: harness főmotor → Soniox (user-kérés).

**🔊 Audio-audit zárás + Soniox async (2026-09-24 este, commit `6fd74e4`)**: (1) a staging Telnyx fiók **már OPUS+G722-t ajánl** (élő API-lekérdezve) — a mért szűksáv a hívó upstream-jéből jön (Telnyx HU DID interconnection), konfiggal nem javítható; a Telnyx portal Call Detail mutatja a lánconkénti kodeket. (2) **Soniox főmotor átirat**: a realtime-WS fájl-betáplálás csonka átiratot adott → átállás a **Soniox async fájl-API-ra** (upload → job → poll → transcript; `SONIOX_API_KEY`, model `stt-async-v5`, v4 fallback) — a valódi 92 mp-es felvételen teljes, tiszta átirat (116 szó, átlag-logprob −0,07). (3) **audio_qc** (`scripts/audio_qc.py`, user-adta): minden hívásrögzítésen fut mindkét csatornára — alapszint: hívó HATÁRESET (szűksáv + 2,6 digitális rés/perc), agent OK. (4) Harness: **await-elve** a hívás végén (a fire-and-forget task a worker kilépésekor elhalt). Nyitva: 5 email-diktálásos teszthívás kiértékelése a javított láncolon; 14/30 nap GDPR-szöveg döntés; prod-deploy checklist (migrációk + env).
